...
def safe_save_model_for_hf_trainer(trainer: transformers.Trainer, output_dir: str):
    ...
    if hasattr(trainer.args, "mm_tunable_parts") and (len(trainer.args.mm_tunable_parts.split(",")) == 2 and ("mm_mlp_adapter" in trainer.args.mm_tunable_parts and "mm_vision_tower" in trainer.args.mm_tunable_parts)):
        check_save_mm_adapter_tunnable_and_vision_tower = True
    else:
        check_save_mm_adapter_tunnable_and_vision_tower = False
    ...
    rank0_print(f"Save projectors and vision model: {check_save_mm_adapter_tunnable_and_vision_tower}")
    if check_save_mm_adapter_tunnable_and_vision_tower:
        keys_to_match = ["mm_projector", "vision_resampler"]
        if getattr(trainer.args, "use_im_start_end", False):
            keys_to_match.extend(["embed_tokens", "embed_in"])
        ...
        if trainer.args.local_rank == 0 or trainer.args.local_rank == -1:
            if current_folder.startswith('checkpoint-'):
                ...
                os.makedirs(mm_projector_folder, exist_ok=True)
                torch.save(vision_weight_to_save, os.path.join(mm_projector_folder, f'{current_folder}.bin'))
            else:
                torch.save(vision_weight_to_save, os.path.join(output_dir, f'vision_tower.bin'))
        return
...
def preprocess_deepseekcoder(
    ...
    assert conv.sep_style == conversation_lib.SeparatorStyle.DeepSeekCoder
...
def get_model(model_args, training_args, bnb_model_from_pretrained_args):
    ...
    if model_args.model_class_name is not None:
        ...
        if "mixtral" in model_args.model_name_or_path.lower():
            ...
            rank0_print('Using DeepseekCoder')
...
def train(attn_implementation=None):
    ...
    if model_args.version == "v0":
        ...
        if tokenizer.unk_token is not None:
            if 'deepseek-coder' not in model_args.model_name_or_path.lower():
                tokenizer.pad_token = tokenizer.unk_token
    ...
    rank0_print("zxl count", count_model_parameters(model))
