...
VISION_MODEL_VERSION="/mnt/afs/hfmodel/siglip-so400m-patch14-384"
VISION_MODEL_VERSION_CLEAN=$(basename $VISION_MODEL_VERSION)
############### Pretrain ################
PROMPT_VERSION=plain
DATA="/data/home/zhaoxuanle/ChartCoder/pt_data.json"
...
run_cmd="${OPTIONS_NCCL} deepspeed --num_nodes ${NUM_WORKERS} --num_gpus ${NUM_GPUS_PER_WORKER} --master_port ${MASTER_PORT} --hostfile ${HOST_FILE_PATH} \
    ...
    --bf16 True \
    --output_dir /mnt/afs/zxl/ChartCoder/checkpoints/projectors/${BASE_RUN_NAME} \
    --num_train_epochs 1 \
